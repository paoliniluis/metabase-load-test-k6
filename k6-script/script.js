...
export default function () {
    ...
    // Get the dashboard 1, only the api, not actually rendering it
    let dashboard_1 = http.get(`${host}/api/dashboard/1`)
    checker(dashboard_1)
    logger(dashboard_1)
    sleep(1);

    let query_metadata = http.get(`${host}/api/dashboard/1/query_metadata`)
    checker(query_metadata)
    logger(query_metadata)
    sleep(1);

    let candidates = http.get(`${host}/api/automagic-dashboards/database/1/candidates`)
    checker(candidates)
    logger(candidates)
    sleep(1);

    let search = http.get(`${host}/api/search?models=dataset&limit=1`)
    checker(search)
    logger(search)
    sleep(1);

    let database = http.get(`${host}/api/database`)
    checker(database)
    logger(database)
    sleep(1);

    let collection_tree = http.get(`${host}/api/collection/tree?exclude-other-user-collections=true&exclude-archived=true`)
    checker(collection_tree)
    logger(collection_tree)
    sleep(1);

    let bookmark = http.get(`${host}/api/bookmark`)
    checker(bookmark)
    logger(bookmark)
    sleep(1);
    
    let recents = http.get(`${host}/api/activity/recents?context=views`)
    checker(recents)
    logger(recents)
    sleep(1);

    let popular = http.get(`${host}/api/activity/popular_items`)
    checker(popular)
    logger(popular)
    sleep(1);
    ...
    let tables = http.get(`${host}/api/database/1/schema/public?include_hidden=true`)
    ...
    tables.map(table => {
        ...
        sleep(0.2)
        let datasetCall = http.post(dataset, JSON.stringify({"database":1,"query":{"source-table":table},"type":"query","parameters":[]}), params)
        checker(datasetCall)
